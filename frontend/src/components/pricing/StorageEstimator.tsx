    ...
    return (
        <Container className="min-h-screen max-h-max gap-8" maxWidth={false} component={Stack}>
            <Stack className="flex-[200px] grow-0 shrink-0 items-center justify-center gap-4">
                <Typography className={`${theme === "light" ? 'text-zinc-700' : 'text-zinc-300'}`} variant="h4">Media Storage Estimator</Typography>
                <Typography className="text-zinc-500 text-center">Don't know which plan to decide? Use our media storage estimator to visualize easier</Typography>
            </Stack>
            ...
            <Stack className={`flex-[100px] grow-0 shrink-0 items-center justify-evenly border-b ${theme === "light" ? 'border-zinc-300' : 'border-zinc-700'}`} direction={"row"}>
                <Stack className="flex-1/2 items-center justify-center gap-1">
                ...
                <Stack className="flex-1/2 items-center justify-center gap-1">
                    ...
                    <Typography className={`${theme === "light" ? 'text-zinc-700' : 'text-zinc-300'}`}>{totalSize <= 15 && averagePhotoSize <= 100 && averageVideoSize * 1024 <= 100 ? "Free" : totalSize <= 500 && averageVideoSize < 50  ? "Plus" : "Pro"}</Typography>
                </Stack>
            </Stack>
            <Stack className="flex-[100px] grow-0 shrink-0 items-center justify-center flex-wrap gap-8" direction={"row"}>
