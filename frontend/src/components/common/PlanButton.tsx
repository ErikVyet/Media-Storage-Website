...
function PlanButton({ plan }: PlanButtonProps) {
    return (
        <Tooltip title={"Subscription"}>
            <Box className={`relative px-3.5 py-1 border rounded-sm cursor-pointer ${plan === "free" ? 'border-yellow-800' : plan === "plus" ? 'border-green-800' : 'border-blue-600'}`} component={Link} to={"/pricing"} draggable={false}>
